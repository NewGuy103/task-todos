...
class SubjectItemController(QObject):
    ...
    def add_subject(self):
        text, ok = QInputDialog.getText(self.mw_parent, "Task to-dos - Add subject", "Enter subject name:")
        if text in self.app_data.subjects:
            QMessageBox.warning(self.mw_parent, "Task to-dos - Add subject", "Provided subject already exists.")
            return
    ...
    def remove_subject(self):
        if not self.app_data.subjects:
            QMessageBox.information(self.mw_parent, "Task to-dos - Remove subject", "There are no subjects to remove.")
            return

        item, ok = QInputDialog.getItem(
            self.mw_parent, "Task to-dos - Remove subject", "Choose a subject to delete:", self.app_data.subjects
        )
        if item and ok:
...
class TasksItemController(QObject):
    ...
    def add_task_accepted(self, task: EditedSubjectTask):
        new_task = SubjectTask(task_id=uuid.uuid4(), completed=False, **task.model_dump())
    ...
    def complete_task_triggered(self):
        ...
        if not data.completed:
            QMessageBox.information(
                self.mw_parent, "Task to-dos - Add task", f"Task '{new_model.task}' successfully completed!"
            )
    ...
    def edit_task_accepted(self, task: EditedTaskWithID):
        ...
        edited_task = SubjectTask(**task.model_dump())
...
class SortAndFilterController(QObject):
    ...
    def setup(self):
        self._sort_filter_state = SortAndFilterState(
            sort_state=SortState(sort_mode=SortingModeEnum.disabled, sort_option=None),
            filter_state=FilterState(
                subject=False,
                category=False,
                deadline=DeadlineFilterState(
                    enabled=False, not_before=datetime.now(UTC), not_after=datetime.now(UTC) + timedelta(weeks=12)
                ),
    ...
    def _filter_tasks(self):
        ...
        if deadline.enabled:
            ...
            final_tasks = [task for task in final_tasks if deadline.not_before < task.deadline < deadline.not_after]
            logger.debug(
                "Filtered for tasks with datetime not before '%s' and not after '%s', received %d tasks",
                deadline.not_before,
                deadline.not_after,
                len(final_tasks),
            )
...
class TaskInfoDialog(QDialog):
    ...
    def accept(self):
        ...
        aware_deadline: datetime = py_deadline.astimezone(UTC)
        category = self.ui.categoryComboBox.currentIndex()

        data = EditedSubjectTask(subject=subject, category=category, deadline=aware_deadline, task=task)
        ...
        if self._emit_as == EmitDialogInfoAs.add:
            ...
            with_id_data = EditedTaskWithID(
                task_id=self._data.task_id, completed=self._data.completed, **data.model_dump()
            )
