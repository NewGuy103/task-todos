...
from ..models import (
    BuiltinCategories, DeadlineFilterState, DeadlineSortingEnum, 
    SortState, FilterState, SortAndFilterState, AlphabeticalSortingEnum,
    SortingModeEnum, FilterCompletedTasksEnum
)
...
class SortFilterOptionsController(QObject):
    ...
    def setup(self):
        ...
        future_dt = datetime.now(timezone.utc) + timedelta(weeks=12)  # 3 mo.
    ...
    def update_sort_filter(self):
        not_before_date = self.ui.notBeforeDeadlineDateTimeEdit.dateTime().toPython().astimezone(timezone.utc)
        not_after_date = self.ui.notAfterDeadlineDateTimeEdit.dateTime().toPython().astimezone(timezone.utc)
        ...
        sort_state = SortState(
            sort_mode=self.ui.sortingModeComboBox.currentIndex(),
            sort_option=idx if idx != -1 else None
        )
